...
// Beschrijving opslaan (NL + EN)
document.querySelectorAll('.save-desc').forEach(button => {
  button.addEventListener('click', async e => {
    const wrapper = e.target.closest('.image-wrapper');
    const id = wrapper.dataset.id;

    const text_nl = wrapper.querySelector('.info-text-nl').value;
    const text_en = wrapper.querySelector('.info-text-en').value;

    const response = await fetch('/utrechtsarchief-module6.1/admin/save_description.php', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id, text_nl, text_en })
    });
